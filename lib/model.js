...
var Model = function(name, attributes, options) {
  ...
  _.each(options.validate, function(validator, validatorType) {
    if (_.contains(_.keys(attributes), validatorType)) {
...
var addOptionalClassMethods = function() {
  var self = this;
  _.each(this.options.classMethods || {}, function(fct, name) { self[name] = fct; });
};
...
Model.prototype.init = function(modelManager) {
  ...
  this._isReadOnlyAttribute = _.memoize(function(key) {
    return self._hasReadOnlyAttributes && self._readOnlyAttributes.indexOf(key) !== -1;
  });

  if (this.options.instanceMethods) {
    _.each(this.options.instanceMethods, function(fct, name) {
...
Model.prototype.refreshAttributes = function() {
  ...
  _.each(['get', 'set'], function(type) {
    var opt = type + 'terMethods'
      , funcs = _.clone(_.isObject(self.options[opt]) ? self.options[opt] : {})
    ...
    _.each(self.rawAttributes, function(options, attribute) {
  ...
  _.each(this.rawAttributes, function(definition, name) {
    ...
    if (definition.hasOwnProperty('defaultValue')) {
      self._defaultValues[name] = _.partial(Utils.toDefaultValue, definition.defaultValue);
    }
  ...
  this.Instance.prototype._isAttribute = _.memoize(function(key) {
  ...
  this._isPrimaryKey = _.memoize(function(key) {
...
Model.prototype.sync = function(options) {
  ...
  return Promise.try(function () {
    ...
    indexes = _.filter(self.options.indexes, function (item1) {
      return !_.any(indexes, function (item2) {
...
Model.prototype.aggregate = function(attribute, aggregateFunction, options) {
  options = _.extend({ attributes: [] }, options || {});
  conformOptions(options, this);
  this.$injectScope(options);
...
Model.prototype.findCreateFind = function(options) {
  ...
  var values = _.clone(options.defaults) || {};
  if (_.isPlainObject(options.where)) {
    values = _.defaults(values, options.where);
  }
...
Model.prototype.bulkCreate = function(records, options) {
  ...
  if (options.updateOnDuplicate) {
    ...
    var updatableFields = _.pull(Object.keys(this.tableAttributes), 'createdAt');
  ...
  return Promise.try(function() {
    ...
    if (options.individualHooks) {
      ...
      return Promise.map(instances, function(instance) {
        var individualOptions = _.clone(options);
...
Model.prototype.update = function(values, options) {
  ...
  return Promise.try(function() {
    ...
    if (options.validate) {
      ...
      options.skip = _.difference(Object.keys(self.attributes), Object.keys(values));
...
_.extend(Model.prototype, associationsMixin);
Hooks.applyTo(Model);
