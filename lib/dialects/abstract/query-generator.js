  ...
  */
      ...
      var generateJoinQueries = function(include, parentTable) {
        ...
        if (through) {
                  ...
                  $query = self.selectQuery(topInclude.model.tableName, {
                    ...
                    where: {
                      $join: self.sequelize.asIs([
                        ...
                        self.quoteIdentifier(topInclude.model.name) + '.' + self.quoteIdentifier(topInclude.association.targetField)
                      ].join(' = '))
                    },
                    limit: 1,
                    includeIgnoreAttributes: false
          ...
          if (subQuery && include.subQueryFilter) {
            ...
            associationWhere[association.targetField] = {
              $raw: self.quoteTable(parentTable) + '.' + self.quoteIdentifier(association.sourceField)
            };
            ...
            $query = self.selectQuery(include.model.getTableName(), {
              attributes: [association.targetField],
              where: {
                $and: [
                  associationWhere,
                  include.where || {}
  ...
  joinIncludeQuery: function(options) {
    var subQuery = options.subQuery
      ...
      , asLeft
      , attrLeft = association.sourceKey
      , fieldLeft = association.sourceField

      /* Attributes for the right side */
      , right = include.model
      , asRight = include.as
      , tableRight = right.getTableName()
      , fieldRight = association.targetField;
