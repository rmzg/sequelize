...
function addForeignKeyConstraints (newAttribute, source, target, options, key) {
  ...
  if (options.foreignKeyConstraint || options.onDelete || options.onUpdate) {
    ...
    if (primaryKeys.length === 1) {
			// Attempt to preserve previous reference/onDelete/onUpdate settings
			// This matches current behaviour, not sure if bug.
			if (!newAttribute.references) {
				if (source.$schema) {
      ...
      newAttribute.onDelete = newAttribute.onDelete || options.onDelete;
      newAttribute.onUpdate = newAttribute.onUpdate || options.onUpdate;
    }
  }
}
