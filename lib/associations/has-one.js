...
var HasOne = function(srcModel, targetModel, options) {
  ...
  if (this.as) {
	...
	var default_target = this.targetKey = Utils.camelizeIf(
		Utils.underscoredIf(Utils.singularize(this.source.name), this.target.options.underscored)
			+ '_' + this.source.primaryKeyAttribute,
		!this.source.options.underscored
	);
	var default_source = this.source.primaryKeyAttribute;
	var default_target_type = this.options.keyType || this.source.rawAttributes[this.source.primaryKeyAttribute].type;

	this.targetAttribute = this.handleKeyOption( options.targetKey || options.foreignKey, this.target, default_target, default_target_type );
		this.targetKey = this.targetAttribute.name || this.targetAttribute.field;
		this.targetField = this.targetAttribute.field || this.targetKey;

	this.sourceAttribute = this.handleKeyOption( options.sourceKey, this.source, default_source );
		this.sourceKey = this.sourceAttribute.name || this.sourceAttribute.field;
