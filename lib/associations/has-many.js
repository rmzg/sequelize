...
//var setForeignKeys = function(options) {
	//this.targetKeyAttribute = {};
	 ////Foreign key setup
	//if (_.isObject(options.foreignKey)) {
		//this.targetKeyAttribute = options.foreignKey;
		//this.targetKey = this.targetKeyAttribute.name || this.targetKeyAttribute.fieldName;
	//} else if (options.foreignKey) {
		//this.targetKey = options.foreignKey;
	//}

	//if (!this.targetKey) {
		//this.targetKey = Utils.camelizeIf(
			//[
	...
	//if (this.target.rawAttributes[this.targetKey]) {
	...
	//} else {
		...
		//this.sourceField = this.source.rawAttributes[this.sourceKey].field;
	//}
//};
/**
...
var HasMany = function(source, target, options) {
  ...
  if (this.as) {
	...
	/* This association doesn't use the this.as alias for default target.. */
...
HasMany.prototype.injectAttributes = function() {
  ...
  if (this.options.constraints !== false) {
    constraintOptions.onDelete = constraintOptions.onDelete || (this.target.rawAttributes[this.targetKey].allowNull ? 'SET NULL' : 'CASCADE');
    constraintOptions.onUpdate = constraintOptions.onUpdate || 'CASCADE';
  }
  Helpers.addForeignKeyConstraints(this.target.rawAttributes[this.targetKey], this.source, this.target, constraintOptions, this.sourceField);
...
HasMany.prototype.get = function(instances, options) {
  ...
  if (instances) {
    ...
    where[association.targetKey] = instance.get(association.source.primaryKeyAttribute, {raw: true});
  }
  ...
  return Model.findAll(options).then(function (results) {
    ...
    results.forEach(function (instance) {
      result[instance.get(association.targetKey, {raw: true})].push(instance);
    });
...
HasMany.prototype.set = function(sourceInstance, targetInstances, options) {
  ...
  return association.get(sourceInstance, _.defaults({
    ...
    if (obsoleteAssociations.length > 0) {
      update = {};
      update[association.targetKey] = null;
    ...
    if (unassociatedObjects.length > 0) {
      ...
      update = {};
      update[association.targetKey] = sourceInstance.get(association.source.primaryKeyAttribute);
...
HasMany.prototype.remove = function(sourceInstance, targetInstances, options) {
  ...
  where[association.targetKey] = sourceInstance.get(association.source.primaryKeyAttribute);
...
HasMany.prototype.create = function(sourceInstance, values, options) {
  ...
  values[association.targetKey] = sourceInstance.get(association.source.primaryKeyAttribute);
